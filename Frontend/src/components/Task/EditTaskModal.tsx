...
export default function EditTaskModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30 px-4">
      <div ref={modalRef} className="relative w-full max-w-[520px] rounded-[10px] bg-white p-8 shadow-2xl">
        <button onClick={onClose} className="absolute right-6 top-6 text-2xl text-gray-400 transition-all hover:text-black">
          <X size={22} />
        </button>

        <h2 className="mb-10 text-[26px] text-[#1f1f1f]">Modifier une tâche</h2>
        ...
        <div className="space-y-7">
            ...
            <div className="rounded-[12px] border border-[#e7e7e7] p-4">
              ...
              <div className="flex flex-wrap gap-3">
                {members.map((member: any) => (
                  <button key={member.user.id} type="button" onClick={() => toggleAssignee(member.user.id)} 
                  className={`rounded-full px-4 py-2 text-[14px] transition-all ${assigneeIds.includes(member.user.id) ? "bg-[#d45d00] text-white" : "bg-[#f3f3f5] text-[#1f1f1f]"}`}>
          ...
          <button onClick={handleUpdateTask} disabled={loading || assigneeIds.length === 0} className="mt-6 h-[58px] rounded-[14px] bg-[#1f1f1f] px-8 text-[16px] font-medium text-white transition-all hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">

    ...
    <div className="min-h-screen flex bg-[#f3f3f3] overflow-hidden">
      ...
      <div className="relative flex w-full items-center justify-center bg-[#f3f3f3] px-8 lg:w-[39%]">
        ...
        <div className="absolute left-1/2 top-20 -translate-x-1/2">
          <Image src={logoAbricot}
            alt="Logo Abricot"
            className="h-auto w-[320px]" priority />
        </div>

        <div className="flex min-h-[600px] w-full max-w-[360px] flex-col">
          <h1 className="mt-4 mb-14 text-center text-[58px] font-bold leading-none text-[#d45d00]">Connexion</h1>

          <form onSubmit={handleLogin}
            className="flex flex-col gap-6">
            {/* Email */}
            <div>
              <label htmlFor="email" className="mb-3 block text-[18px] text-[#1d1d1d]">Email</label>
            ...
            <div>
              <label htmlFor="password"
                className="mb-3 block text-[18px] text-[#1d1d1d]">Mot de passe</label>
            ...
            {/* Message d'erreur */}
            {error && <p role="alert" aria-live="assertive" className="text-[15px] text-red-600">{error}</p>}

            {/* Bouton de connexion */}
            <button type="submit" disabled={loading} aria-busy={loading} className="h-[58px] w-full rounded-[12px] bg-[#1f1f23] text-[24px] text-white transition-all duration-200 hover:bg-black disabled:cursor-not-allowed disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-[#d45d00] focus:ring-offset-2">
          ...
          <div className="mt-auto text-center">
            <button type="button" className="rounded-[4px] text-[18px] font-semibold text-[#c45100] underline focus:outline-none focus:ring-2 focus:ring-[#d45d00] focus:ring-offset-2">
              Mot de passe oublié ?
            </button>
            <div className="mt-10 flex items-center justify-center gap-2 whitespace-nowrap text-[20px]">
              <span className="text-[#1d1d1d]">Pas encore de compte ?</span>
              <button type="button" onClick={() => router.push("/register")} className="rounded-[4px] font-semibold text-[#c45100] underline focus:outline-none focus:ring-2 focus:ring-[#d45d00] focus:ring-offset-2">
                Créer un compte
              </button>
            </div>
          </div>
      ...
      <div className="hidden h-screen lg:block lg:w-[61%]">
        <Image src={bannerLogin} alt="Illustration de la page de connexion" className="h-full w-full object-cover" priority />
      </div>
    </div>
  );
}